...
# Discovery/size/deletion logic lives in the GUI-neutral Utils.prefix_manager
# (shared with the Qt prefix manager); aliased to the old private names.
from Utils.prefix_manager import (  # noqa: E402
    WINE_PREFIX_TOOLS as _WINE_PREFIX_TOOLS,  # noqa: F401
    PrefixEntry,
    classify_location as _classify_location,  # noqa: F401
    enumerate_prefixes as _enumerate_prefixes_neutral,
    fmt_size as _fmt_size,
    get_dir_size as _get_dir_size,
    is_deletable_prefix as _is_deletable_prefix,
    scan_root_for_prefixes as _scan_root_for_prefixes,  # noqa: F401
)


def _enumerate_prefixes() -> list[PrefixEntry]:
    """Neutral discovery, fed with the Tk app's configured-games registry
    (for custom staging roots living outside Profiles/)."""
    try:
        from gui.game_helpers import _GAMES  # type: ignore
        games = dict(_GAMES)
    except Exception:
        games = {}
    return _enumerate_prefixes_neutral(games)
...
# _is_deletable_prefix moved to Utils.prefix_manager (imported above).
